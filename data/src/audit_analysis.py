...
def add_quality_metrics(df):
    ...
    df["sla_ready_score"] = df["average_score"].apply(
        lambda score: "Meets Quality Bar" if score >= 4 else "Needs QA Review"
    )
    return df


def summarize_overall_quality(df):
    """Create overall project-level quality metrics."""
    total_responses = len(df)
    pass_rate = (df["final_rating"].eq("Pass").mean() * 100).round(1)
    hallucination_rate = (df["hallucination_flag"].eq("Yes").mean() * 100).round(1)
    average_quality_score = df["average_score"].mean().round(2)
...
def summarize_by_model(df):
    ...
    return (
        ...
        .agg(
            ...
            pass_rate=("final_rating", lambda x: (x.eq("Pass").mean() * 100).round(1)),
            hallucination_rate=(
                "hallucination_flag",
                lambda x: (x.eq("Yes").mean() * 100).round(1),
            ),
        )
        .round(2)
        .reset_index()
...
def summarize_task_types(df):
    """Compare quality patterns across task types."""
    return (
...
def save_outputs(df):
    ...
    df.to_csv(OUTPUT_DIR / "cleaned_ai_response_audit.csv", index=False)
    summarize_overall_quality(df).to_csv(
        OUTPUT_DIR / "overall_quality_summary.csv", index=False
    )
    summarize_by_model(df).to_csv(
        OUTPUT_DIR / "model_performance_summary.csv", index=False
    )
    summarize_failure_types(df).to_csv(
        OUTPUT_DIR / "failure_type_summary.csv", index=False
    )
    summarize_task_types(df).to_csv(
        OUTPUT_DIR / "task_type_summary.csv", index=False
    )
...
def main():
    ...
    print("\nAnalysis complete. Summary files saved to the outputs folder.")
